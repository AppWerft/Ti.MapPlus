...
public class HeatmapTileProvider implements TileProvider {
	...
	public Tile getTile(int x, int y, int zoom) {
		...
		if (minX < 0) {
			...
			Bounds overlapBounds = new Bounds(minX + WORLD_WIDTH, WORLD_WIDTH,
					minY, maxY);
			xOffset = -WORLD_WIDTH;
		...
		Bounds paddedBounds = new Bounds(mBounds.minX - padding, mBounds.maxX
				+ padding, mBounds.minY - padding, mBounds.maxY + padding);
		...
		// Quantize points
		double[][] intensity = new double[TILE_DIM + mRadius * 2][TILE_DIM
				+ mRadius * 2];
	 ...
	 */
	static double getMaxValue(Collection<WeightedLatLng> points, Bounds bounds,
			int radius, int screenDim) {
		...
		double boundsDim = (maxX - minX > maxY - minY) ? maxX - minX : maxY
				- minY;
