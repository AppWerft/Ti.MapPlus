...
public class Renderer {
	 ...
	 * @param context
	 *            context needed to add info windows
	 */
	 ...
	 *            map to place objects on
	 * @param features
	 *            contains a hashmap of features and objects that will go on the
	 *            map
	 */
	 ...
	 *
	 * @param layerOnMap
	 *            contains true if the layer should be set to visible and false
	 *            otherwise
	 */
	...
	/**
	 * Gets a Feature for the given map object, which is a Marker, Polyline or
	 * Polygon.
	 *
	 * @param mapObject
	 ...
	 *
	 * @return mStylesRenderer hashmap containing styles for KML placemarks
	 *         (String, KmlStyle)
	 */
	 ...
	 * Gets the styles for KML placemarks
	 *
	 * @return mStyleMaps hashmap containing styles for KML placemarks (String,
	 *         String)
	 */
	...
	/**
	 * Gets the images cache which is needed to download GroundOverlays and
	 * Marker Icon images
	 *
	 * @return mImagesCache
	 */
	...
	/**
	 * Obtains the styleUrl from a placemark and finds the corresponding style
	 * in a list
	 *
	 * @param styleId
	 ...
	 * 
	 * @param feature
	 *            Feature to be added onto the map
	 * @param object
	 *            Corresponding map object to this feature
	 */
	 ...
	 * 
	 * @param styles
	 *            hashmap of strings and KmlStyles to be added to
	 *            mStylesRenderer
	 */
	...
	/**
	 * Checks for each style in the feature and adds a default style if none is
	 * applied
	 *
	 * @param feature
	 ...
	 *            hashmap of styles
	 * @param styleMaps
	 *            hashmap of style maps
	 * @param features
	 *            hashmap of features
	 * @param folders
	 *            array of containers
	 * @param groundOverlays
	 *            hashmap of ground overlays
	 */
		...
		if (mLayerOnMap) {
			...
			if (feature.hasGeometry()) {
				...
				if (feature instanceof KmlPlacemark) {
					...
					KmlStyle inlineStyle = ((KmlPlacemark) feature)
							.getInlineStyle();
					mapObject = addKmlPlacemarkToMap((KmlPlacemark) feature,
							geometry, style, inlineStyle, isPlacemarkVisible);
				} else {
					mapObject = addGeoJsonFeatureToMap(feature,
							feature.getGeometry());
				}
			}
		}
	...
	/**
	 * Given a Marker, Polyline, Polygon or an array of these and removes it
	 * from the map
	 *
	 * @param mapObject
	 *            map object or array of map objects to remove from the map
	 */
	...
	/**
	 * Adds a new object onto the map using the Geometry for the coordinates and
	 * the Feature for the styles. (used for GeoJson)
	 *
	 * @param feature
	 *            feature to get geometry style
	 * @param geometry
	 *            geometry to add to the map
	 */
	protected Object addGeoJsonFeatureToMap(Feature feature, Geometry geometry) {
		...
		case "LineString":
			...
			if (feature instanceof GeoJsonFeature) {
				polylineOptions = ((GeoJsonFeature) feature)
						.getPolylineOptions();
			} else if (feature instanceof KmlPlacemark) {
				polylineOptions = ((KmlPlacemark) feature).getPolylineOptions();
			}
			return addLineStringToMap(polylineOptions,
		...
		case "MultiPoint":
			return addMultiPointToMap(
					((GeoJsonFeature) feature).getPointStyle(),
					(GeoJsonMultiPoint) geometry);
		case "MultiLineString":
			return addMultiLineStringToMap(
					((GeoJsonFeature) feature).getLineStringStyle(),
					((GeoJsonMultiLineString) geometry));
		case "MultiPolygon":
			return addMultiPolygonToMap(
					((GeoJsonFeature) feature).getPolygonStyle(),
	...
	/**
	 * Adds a single geometry object to the map with its specified style (used
	 * for KML)
	 *
	 * @param geometry
	 *            defines the type of object to add to the map
	 * @param style
	 *            defines styling properties to add to the object when added to
	 *            the map
	 * @return the object that was added to the map, this is a Marker, Polyline,
	 *         Polygon or an array of either objects
	 */
	protected Object addKmlPlacemarkToMap(KmlPlacemark placemark,
			Geometry geometry, KmlStyle style, KmlStyle inlineStyle,
			boolean isVisible) {
		String geometryType = geometry.getGeometryType();
		...
		if (hasDrawOrder) {
			try {
				drawOrder = Float
						.parseFloat(placemark.getProperty("drawOrder"));
		...
		case "LineString":
			...
			if (inlineStyle != null) {
				...
				polylineOptions.color(KmlStyle
						.computeRandomColor(polylineOptions.getColor()));
			}
			Polyline polyline = addLineStringToMap(polylineOptions,
					(LineString) geometry);
		...
		case "Polygon":
			...
			if (inlineStyle != null) {
				...
				polygonOptions.fillColor(KmlStyle
						.computeRandomColor(polygonOptions.getFillColor()));
			}
			Polygon polygon = addPolygonToMap(polygonOptions,
			...
			return polygon;
		case "MultiGeometry":
			return addMultiGeometryToMap(placemark,
					(KmlMultiGeometry) geometry, style, inlineStyle, isVisible);
		}
		return null;
	}
	 ...
	 *
	 * @param markerOptions
	 *            contains relevant styling properties for the Marker
	 * @param point
	 *            contains coordinates for the Marker
	 ...
	 *
	 * @param markerOptions
	 *            marker options object to add inline styles to
	 * @param inlineStyle
	 *            inline styles to apply
	 * @param markerUrlIconUrl
	 *            default marker icon URL from shared style
	 */
	private void setInlinePointStyle(MarkerOptions markerOptions,
			KmlStyle inlineStyle, String markerUrlIconUrl) {
		...
		if (inlineStyle.isStyleSet("hotSpot")) {
			markerOptions.anchor(inlineMarkerOptions.getAnchorU(),
					inlineMarkerOptions.getAnchorV());
		}
	 ...
	 *            contains relevant styling properties for the Polyline
	 * @param lineString
	 *            contains coordinates for the Polyline
	...
	/**
	 * Sets the inline linestring style by copying over the styles that have
	 * been set
	 *
	 * @param polylineOptions
	 *            polygon options object to add inline styles to
	 * @param inlineStyle
	 *            inline styles to apply
	 */
	private void setInlineLineStringStyle(PolylineOptions polylineOptions,
			KmlStyle inlineStyle) {
		PolylineOptions inlinePolylineOptions = inlineStyle
				.getPolylineOptions();
		...
		if (inlineStyle.isLineRandomColorMode()) {
			polylineOptions.color(KmlStyle
					.computeRandomColor(inlinePolylineOptions.getColor()));
		}
	}
	 ...
	 * @param polygon
	 *            contains coordinates for the Polygon
	 * @return Polygon object created from given DataPolygon
	 */
	protected Polygon addPolygonToMap(PolygonOptions polygonOptions,
			DataPolygon polygon) {
		...
		List<List<LatLng>> innerBoundaries = polygon
	...
	/**
	 * Sets the inline polygon style by copying over the styles that have been
	 * set
	 *
	 * @param polygonOptions
	 *            polygon options object to add inline styles to
	 ...
	 *            inline styles to apply
	 */
	private void setInlinePolygonStyle(PolygonOptions polygonOptions,
			KmlStyle inlineStyle) {
		...
		if (inlineStyle.hasOutline()) {
			if (inlineStyle.isStyleSet("outlineColor")) {
				polygonOptions.strokeColor(inlinePolygonOptions
						.getStrokeColor());
			}
			if (inlineStyle.isStyleSet("width")) {
				polygonOptions.strokeWidth(inlinePolygonOptions
						.getStrokeWidth());
			}
		}
		if (inlineStyle.isPolyRandomColorMode()) {
			polygonOptions.fillColor(KmlStyle
					.computeRandomColor(inlinePolygonOptions.getFillColor()));
		}
	}

	/**
	 * Adds all Geometry objects stored in the GeoJsonGeometryCollection onto
	 * the map. Supports recursive GeometryCollections.
	 *
	 * @param feature
	 *            contains relevant styling properties for the Geometry inside
	 *            the GeoJsonGeometryCollection
	 * @param geoJsonGeometries
	 *            contains an array of Geometry objects
	 * @return array of Marker, Polyline, Polygons that have been added to the
	 *         map
	 */
	private ArrayList<Object> addGeometryCollectionToMap(
			GeoJsonFeature feature, List<Geometry> geoJsonGeometries) {
	...
	/**
	 * Gets the visibility of the placemark if it is specified. A visibility
	 * value of "1" corresponds as "true", a visibility value of "0" corresponds
	 * as false. If the visibility is not set, the method returns "true".
	 *
	 * @param feature
	 *            Feature to obtain visibility from.
	...
	/**
	 * Adds all the geometries within a KML MultiGeometry to the map. Supports
	 * recursive MultiGeometry. Combines styling of the placemark with the
	 * coordinates of each geometry.
	 *
	 * @param multiGeometry
	 *            contains array of geometries for the MultiGeometry
	 * @param urlStyle
	 *            contains relevant styling properties for the MultiGeometry
	 * @return array of Marker, Polyline and Polygon objects
	 */
	private ArrayList<Object> addMultiGeometryToMap(KmlPlacemark placemark,
			KmlMultiGeometry multiGeometry, KmlStyle urlStyle,
			KmlStyle inlineStyle, boolean isContainerVisible) {
		...
		for (Geometry kmlGeometry : kmlObjects) {
			mapObjects.add(addKmlPlacemarkToMap(placemark, kmlGeometry,
					urlStyle, inlineStyle, isContainerVisible));
		}
		return mapObjects;
	}

	/**
	 * Adds all GeoJsonPoint objects in GeoJsonMultiPoint to the map as multiple
	 * Markers
	 *
	 * @param pointStyle
	 *            contains relevant styling properties for the Markers
	 * @param multiPoint
	 *            contains an array of GeoJsonPoints
	...
	private ArrayList<Marker> addMultiPointToMap(GeoJsonPointStyle pointStyle,
		...
		for (GeoJsonPoint geoJsonPoint : multiPoint.getPoints()) {
			markers.add(addPointToMap(pointStyle.toMarkerOptions(),
					geoJsonPoint));
		}
		return markers;
	}
	...
	/**
	 * Adds all GeoJsonLineString objects in the GeoJsonMultiLineString to the
	 * map as multiple Polylines
	 *
	 * @param lineStringStyle
	 *            contains relevant styling properties for the Polylines
	 * @param multiLineString
	 *            contains an array of GeoJsonLineStrings
	...
	private ArrayList<Polyline> addMultiLineStringToMap(
		...
		for (GeoJsonLineString geoJsonLineString : multiLineString
				.getLineStrings()) {
			polylines.add(addLineStringToMap(
	...
	/**
	 * Adds all GeoJsonPolygon in the GeoJsonMultiPolygon to the map as multiple
	 * Polygons
	 *
	 * @param polygonStyle
	 *            contains relevant styling properties for the Polygons
	 * @param multiPolygon
	 *            contains an array of GeoJsonPolygons
	 * @return array of Polygons that have been added to the map
	 */
	private ArrayList<Polygon> addMultiPolygonToMap(
			GeoJsonPolygonStyle polygonStyle, GeoJsonMultiPolygon multiPolygon) {
		ArrayList<Polygon> polygons = new ArrayList<>();
		for (GeoJsonPolygon geoJsonPolygon : multiPolygon.getPolygons()) {
			polygons.add(addPolygonToMap(polygonStyle.toPolygonOptions(),
					geoJsonPolygon));
		}
		return polygons;
	}
	 ...
	 *
	 * @param styleUrl
	 *            The style which we retrieve the icon url from
	 * @param markerOptions
	 *            The marker which is displaying the icon
	 */
	 ...
	 *
	 * @param groundOverlayOptions
	 *            GroundOverlay style options to be added to the map
	 * @return new GroundOverlay object created from the given
	 *         GroundOverlayOptions
	 */
	public GroundOverlay attachGroundOverlay(
			GroundOverlayOptions groundOverlayOptions) {
		return mMap.addGroundOverlay(groundOverlayOptions);
	}

	/**
	 * Sets a marker info window if no <text> tag was found in the KML document.
	 * This method sets the marker title as the text found in the <name> start
	 * tag and the snippet as <description>
	 *
	 * @param style
	 *            Style to apply
	 */
	...
	/**
	 * Creates a new InfoWindowAdapter and sets text if marker snippet or title
	 * is set. This allows the info window to have custom HTML.
	 */
	private void createInfoWindow() {
		mMap.setInfoWindowAdapter(new GoogleMap.InfoWindowAdapter() {
			...
			public View getInfoContents(Marker arg0) {
				...
				// View view =
				// LayoutInflater.from(mContext).inflate(R.layout.amu_info_window,
				// null);
				// TextView infoWindowText = (TextView)
				// view.findViewById(R.id.window);
