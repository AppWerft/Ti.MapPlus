...
public class PolyUtil {
	...
	public static boolean containsLocation(LatLng point, List<LatLng> polygon,
			boolean geodesic) {
		return containsLocation(point.latitude, point.longitude, polygon,
				geodesic);
	}
	 ...
	 * otherwise.
	 */
	public static boolean containsLocation(double latitude, double longitude,
			List<LatLng> polygon, boolean geodesic) {
	 ...
	 * decimation algorithm. Increasing the tolerance will result in fewer
	 * points in the simplified polyline or polygon.
	 *
	 ...
	 *
	 * The time complexity of Douglas-Peucker is O(n^2), so take care that you
	 * do not call this algorithm too frequently in your code.
	 *
	 * @param poly
	 *            polyline or polygon to be simplified. Polygon should be closed
	 *            (i.e., first and last points should have the same latitude and
	 *            longitude).
	 * @param tolerance
	 *            in meters. Increasing the tolerance will result in fewer
	...
	public static List<LatLng> simplify(List<LatLng> poly, double tolerance) {
		...
		if (closedPolygon) {
			// Add a small offset to the last point for Douglas-Peucker on
			// polygons (see #201)
			final double OFFSET = 0.00000000001;
			lastPoint = poly.get(poly.size() - 1);
			// LatLng.latitude and .longitude are immutable, so replace the last
			// point
			poly.remove(poly.size() - 1);
			poly.add(new LatLng(lastPoint.latitude + OFFSET,
					lastPoint.longitude + OFFSET));
		}
		...
		if (closedPolygon) {
			// Replace last point w/ offset with the original last point to
	 ...
	 * the first and last points are the same), and false if it is not
	 * 
	 * @param poly
	 *            polyline or polygon
	 * @return true if the provided list of points is a closed polygon (i.e.,
	 *         the first and last points are the same), and false if it is not
	 */
	 ...
	 */
	public static double distanceToLine(final LatLng p, final LatLng start,
			final LatLng end) {
		if (start.equals(end)) {
		...
		LatLng sa = new LatLng(p.latitude - start.latitude, p.longitude
				- start.longitude);
		LatLng sb = new LatLng(u * (end.latitude - start.latitude), u
