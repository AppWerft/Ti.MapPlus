...
public class BoundingBox {
	...
	private static double[] transformGoogleToCRS(double x, double y, String csr) {
		/*
		 * CoordinateReferenceSystem sourceCrs; try { sourceCrs =
		 * CRS.decode(GOOGLE); Log.d(LCAT,sourceCrs.toWKT());
		 * CoordinateReferenceSystem targetCrs = CRS.decode(csr);
		 * Log.d(LCAT,targetCrs.toWKT()); MathTransform mathTransform =
		 * CRS.findMathTransform(sourceCrs, targetCrs, true); DirectPosition2D
		 * srcDirectPosition2D = new DirectPosition2D( sourceCrs, x, y);
		 * DirectPosition2D destDirectPosition2D = new DirectPosition2D();
		 * mathTransform.transform(srcDirectPosition2D, destDirectPosition2D);
		 * double[] coords = new double[2]; coords[0] = destDirectPosition2D.x;
		 * coords[1] = destDirectPosition2D.y; return coords; } catch
		 * (FactoryException | MismatchedDimensionException | TransformException
		 * e) { Log.d(LCAT,e.getMessage()); e.printStackTrace(); }
		 */
		return null;
	}
	...
	public String getBBox_UTM() {
		String BB = String.valueOf(new Deg2UTM(ymin, xmin).Easting + ","
				+ String.valueOf(new Deg2UTM(ymin, xmin).Northing) + ","
				+ String.valueOf(new Deg2UTM(ymax, xmax).Easting) + ","
				+ String.valueOf(new Deg2UTM(ymax, xmax).Northing));
		return BB;
	}
